...
class RauschFoxpost extends \Opencart\System\Engine\Controller {
	...
	public function orders() {
		...
		elseif (isset($get['sync'])) { 
			...
			foreach ($foxpost_datas as $foxpost_data) {
				...
				if ($error) {
					...
					if ($sid = $this->config->get('shipping_foxpost_order_foxpost_'.$resp['data']['traces'][0]['status'].'_status_id')) {
						$query = $this->db->query("
		...
		elseif (isset($get['create'])) {
			...
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "foxpost_order WHERE foxpost_order_id = " . round($get['create']));
			...
			if ($foxpost_type == "transfer") {
				$data['recipientCity']    = $order_data['shipping_city'];
				$data['recipientZip']     = $order_data['shipping_postcode'];
				$data['recipientAddress'] = $order_data['shipping_address_1']." ".$order_data['shipping_address_2'];
			}
			...
			$data['size']           = $get['size'];
			$data['source']         = "opencart3_".$this->version;
			...
			if ($error) {
				...
				$query = $this->db->query("UPDATE " . DB_PREFIX . "foxpost_order SET foxpost_barcode = '" . $resp['data']['parcels'][0]['clFoxId'] . "', foxpost_size = '" . $resp['data']['parcels'][0]['size'] . "', date_modified = NOW() WHERE foxpost_order_id = " . round($get['create']) );
				...
				if ($this->config->get('shipping_foxpost_order_foxpost_status_id')) {
					$comment = $this->language->get('text_foxpost_tracking') . ": https://foxpost.hu/csomagkovetes/?code=" . $resp['data']['parcels'][0]['clFoxId'];

					$this->load->model('extension/rausch_foxpost/shipping/rausch_foxpost');
					$this->model_extension_shipping_foxpost->addOrderHistory(
						round($foxpost_data['order_id']),
						$this->config->get('shipping_foxpost_order_foxpost_status_id'),
						$comment
					);
				}
		...
		// LIST
			...
			if (isset($get['filter']) && $get['filter'] == 'archive') {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "foxpost_order WHERE foxpost_status IN ('RECIEVE', 'HDRECIEVE', 'HDRETURN', 'INWAREHOUSE', 'COLLECTSENT') ORDER BY " . $orderby . " " . $orderbysub);
				$data['archive']  = true;
			}
			else {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "foxpost_order WHERE foxpost_status NOT IN ('RECIEVE', 'HDRECIEVE', 'HDRETURN', 'INWAREHOUSE', 'COLLECTSENT') ORDER BY " . $orderby . " " . $orderbysub);
			}
