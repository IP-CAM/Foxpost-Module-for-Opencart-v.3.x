...
class RauschFoxpost extends \Opencart\System\Engine\Model {
    ...
    public function getQuote($address) {
        ...
        if (isset($this->session->data['shipping_method'])) {
            ...
            if ($shipping_method_code[0] == "rausch_foxpost" && (!($shipping_method_code[1] == "transfer" || $shipping_method_code[1] == "transfercod"))) {
    ...
    public function getFoxpostId($order_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "rausch_foxpost_order WHERE order_id = '" . (int) $order_id . "'");
        if($query->num_rows) {
            return true;
        }
        return false;
